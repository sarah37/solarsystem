...
d3.select("#vArrowsOn").on("change", function() {
  if (vArrowsOn) {
    ...
    else {d3.selectAll(".vArrowMono").classed("invisible", false)}
  }
})
...
d3.select("#aArrowsOn").on("change", function() {
  if (aArrowsOn) {
    ...
    else {d3.selectAll(".aArrowMono").classed("invisible", false)}
  }
})

d3.select("#explicitOn").on("change", function() {
  if (explicitOn) {
    explicitOn = false;
    d3.select("#resetButton").attr("value", "Reset")
    d3.select("#messHeading").html("Play with the solar system")
  }
  else {
    explicitOn = true;
    d3.select("#resetButton").attr("value", "I fucked up")
    d3.select("#messHeading").html("Fuck up the solar system")
  }
})
...
d3.select("#vLegend")
  ...
  .attr("height", "8px")
  .append("polyline")
  .attr("points", function() {return arrowPoints(0, 4, 40)});
...
var planetData = [
  { "i": 0, "name": "Sun",   "size": 10,  "colour": "#fd1", "m": 1,
    "x": [0, 0], "v": [0, 0], "a": [0, 0]},
  { "i": 1, "name": "Earth", "size": 3, "colour": "#01f", "m": 3*math.pow(10, -6),
    "x": [1, 0], "v": [0, 6.283185307179586], "a": [0, 0]}
  ];
...
var accelerationScale = d3.scale.linear()
            .domain([-50, 50])
            .range([-max*0.2, max*0.2]);
...
// sun gradient
var gradientSun = defs.append("radialGradient")
  .attr("id", "gradientSun")
gradientSun.append("stop")
  .attr("offset", "0%")
...
// earth gradient
var gradientEarth = defs.append("radialGradient")
  .attr("id", "gradientEarth")
  .attr("cx", "22%")
  .attr("cy", "50%")
  .attr("r", "65%");
gradientEarth.append("stop")
  .attr("offset", "0%")
  .attr("stop-color", function() { return d3.rgb("#01f").brighter(1); });
gradientEarth.append("stop")
  .attr("offset", "50%")
  .attr("stop-color", "#01f")
gradientEarth.append("stop")
  .attr("offset", "100%")
  .attr("stop-color", function() { return d3.rgb("#01f").darker(1.5)});
...
all = svg.selectAll("circle")
  ...
  all.append("circle")
  .classed("planet", true)
  .attr("id", function(d) {return "is" + d.name})
  .attr("cx", function(d) {
  ...
  .attr("id", function(d) {return "isVXof" + d.name});

  all.append("polyline")
  .classed("vArrowSplit", true)
  .attr("id", function(d) {return "isVYof" + d.name});  

  all.append("polyline")
  .classed("vArrowMono", true)
  .attr("id", function(d) {return "isVof" + d.name})

  all.append("polyline")
  .classed("aArrowSplit", true)
  .attr("id", function(d) {return "isAXof" + d.name});

  all.append("polyline")
  .classed("aArrowSplit", true)
  .attr("id", function(d) {return "isAYof" + d.name});  

  all.append("polyline")
  .classed("aArrowMono", true)
  .attr("id", function(d) {return "isAof" + d.name})
...
function tickFn(_elapsed) {
  ...
  svg.selectAll(".planet")
    .attr("cx", function(d) {return w/2 + posScale(d.x[0])})
    .attr("cy", function(d) {return h/2 + posScale(d.x[1])});
  ...
  svg.select("#isEarth")
    .attr("transform", function(d) {
        ...
        return "rotate(" + rad2deg(Math.atan2(d.x[1], d.x[0])) + ", " + x + ", " + y + ")"
      });

  if (vArrowsOn) {
    if (splitXY) {
      svg.select("#isVXofEarth")
      ...
      .attr("transform", function(d) {
        ...
        else {return "rotate(90, " + x + "," + y + ")"}
      });
    } // end split XY for v

    if (splitXY == false) {
      svg.select("#isVofEarth")
      .attr("points", function(d) {
        x = w/2 + posScale(d.x[0]);
        y = h/2 + posScale(d.x[1]);
...
function lengthVector(u) {return math.sqrt(math.dot(u,u))};
...
function arrowPoints(x, y, l) {
  if (l <= 7) {
    var points = (x) + " " + (y+(4/7)*l) + ", " + (x+l) + " " + y + ", " + (x) + " " + (y-(4/7)*l);
    return points
  }
  else {
    var p1 = x + " " + (y-1);
...
function unitVector (vector) {
  result = vector.map(function(x) {return x / lengthVector(vector)});
  return result;
}